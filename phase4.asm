...
; ============ GAME STATE ============
gameState    db 0    ; 0 = start screen, 1 = playing, 2 = paused
...
pauseMsg        db 'Do you want to quit? (Y/N)', 0
endMsg          db 'Game Ended', 0
pauseText       db 'Do you want to quit? (Y/N)', 0
...
DrawPauseScreen:
    ...
    ; Draw a rectangle in the center (100x60 pixels at position 110,70)
    mov di, 320 * 70 + 110
    mov cx, 60          ; Height of box
...
dpDrawBorder:
    mov dx, 100
dpBorderLoop:
    mov byte [es:di], 15    ; White border
    inc di
    dec dx
    jnz dpBorderLoop
    jmp dpNextRow
...
dpDrawInside:
    ...
    ; Fill inside with dark gray
    mov dx, 98
dpFillLoop:
    mov byte [es:di], 8     ; Dark gray fill
    inc di
    dec dx
    jnz dpFillLoop
...
dpBoxDone:
    ; Draw pause text "Do you want to quit? (Y/N)" centered in box
    mov di, 320 * 90 + 115
    mov bx, pauseText
    mov al, 15          ; White color
    call DrawText
...
spsLoop:
    ...
    ; Draw pause text "Do you want to quit? (Y/N)" centered
    mov di, 320 * 95 + 82
    mov bx, pauseMsg
    mov al, 15          ; White color
    call DrawText
...
ShowEndScreen:
    ...
    ; Fill screen with purple (color 5)
    xor di, di
    mov cx, 64000       ; 320 * 200 pixels
...
sesLoop:
    ...
    ; Draw end text "Game Ended" centered
    mov di, 320 * 95 + 115
    mov bx, endMsg
    mov al, 15          ; White color
    call DrawText
...
MainLoop:
    ...
    mov al, [gameState]
    cmp al, 2
    je MainLoop         ; Skip updates if paused
    
    ; Check if we're still in start screen
    cmp al, 0
    je MainLoop         ; Skip updates if in start screen
