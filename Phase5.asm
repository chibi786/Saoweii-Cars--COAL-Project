...
BackgroundMusic:
    dw NOTE_C4, 2, NOTE_E4, 2, NOTE_G4, 2, NOTE_E4, 2
    dw NOTE_C4, 2, NOTE_REST, 1, NOTE_D4, 2, NOTE_F4, 2
    dw NOTE_A4, 2, NOTE_F4, 2, NOTE_D4, 2, NOTE_REST, 1
    dw NOTE_E4, 2, NOTE_G4, 2, NOTE_C5, 4, NOTE_REST, 2
    dw 0, 0

GameOverMusic:
    dw NOTE_C5, 2, NOTE_B4, 2, NOTE_A4, 2, NOTE_G4, 2
    dw NOTE_F4, 2, NOTE_E4, 2, NOTE_D4, 2, NOTE_C4, 4
    dw NOTE_REST, 4, 0, 0

CoinSound:
    dw NOTE_E4, 1, NOTE_G4, 1, NOTE_C5, 2, 0, 0

musicEnabled db 1
currentMusicPtr dw BackgroundMusic
musicNoteIndex dw 0
musicDurationCounter dw 0
oldTimerVector dd 0
timerTickCounter db 0
soundEffectActive db 0
soundEffectPtr dw 0
soundEffectIndex dw 0
soundEffectDuration dw 0
...
.nextEnemy:
    ...
    neg cx
.xPositive:
    cmp cx, 20                 ; If distance > 20, no collision
    ja .skipEnemy
    ...
    neg dx
.yPositive:
    cmp dx, 24                 ; If distance > 24, no collision
    ja .skipEnemy
...
.xPositive:
    ...
    neg dx
.yPositive:
    cmp dx, 15                 ; Collision threshold
    ja .skipFuel

    ; ===== FUEL COLLECTED =====
    mov byte [fuelActive + bx], 0  ; Deactivate fuel
    add word [fuel], 20            ; Add 20 fuel
...
; ============ DRAW NUMBER ============
; Draw a 4-digit number at DI position
; AX = number to draw (0-9999)
; DI = screen offset
; Uses color from previous DrawText call
...
.divLoop:
    ...
    ; Now draw digits in correct order
.drawLoop:
    pop ax
    add al, '0'             ; Convert to ASCII
    
    ; Draw the character at current position
    mov bl, al              ; Save character in BL for DrawChar
    mov al, bl
    call DrawChar           ; This will use the color from BL (set by DrawText)
    
    add di, 6               ; Move to next character position
    loop .drawLoop
...
DrawFuelBar:
    ...
    ; Clear fuel area first (top right, 100 pixels wide, 15 pixels tall)
    mov di, 320 * 5 + 210    ; Moved further right
    mov cx, 15               ; Taller bar (15 pixels)
.clearLoop:
    push di
    mov ax, 100              ; Wider area (100 pixels)
    mov bx, ax
.clearInner:
    ...
    mov di, 320 * 5 + 210    ; Moved right to match bar
    mov bx, fuelText
    mov al, 15                ; White
    call DrawText
    
    ; Draw fuel bar as pixels (fuel = 0-100 pixels, no division)
    mov ax, [fuel]
    mov cx, ax                ; Use full fuel value (0-100)
    mov di, 320 * 8 + 210     ; Below "Fuel:" text
    
    ; Draw multiple rows for thicker bar (5 pixels tall)
    mov dx, 5                 ; 5 rows thick
.drawRows:
    push di
    push cx
.drawBar:
...
DecreaseFuel:
    ...
    ; Decrease fuel every 3 frames (smooth and visible)
    mov al, [frameCounter]
    mov bl, 100
    xor ah, ah
    div bl
...
hiInGame:
    ...
    je hiMoveRight
    cmp bh, 48h         ; Up arrow scan code
    je hiMoveUp
    cmp bh, 50h         ; Down arrow scan code
    je hiMoveDown
...
DrawPauseScreen:
    ...
    ; First, redraw the complete game state
    call DrawStripes
    call LoadCar
    call DrawScore
    call DrawFuelBar
...
InitMusic:
    ...
    mov ax, 0x3508
    int 0x21
    mov word [oldTimerVector], bx
    mov word [oldTimerVector + 2], es
    
    push ds
    mov dx, MusicTimerISR
    mov ax, cs
    mov ds, ax
    mov ax, 0x2508
    int 0x21
    ...
    mov word [musicNoteIndex], 0
    mov word [musicDurationCounter], 0
    mov byte [soundEffectActive], 0
    mov byte [timerTickCounter], 0
...
MusicTimerISR:
    ...
    inc byte [timerTickCounter]
    cmp byte [timerTickCounter], 3
    jb .callOldISR
    mov byte [timerTickCounter], 0
...
UpdateBackgroundMusic:
    ...
    mov ax, [musicDurationCounter]
    cmp ax, 0
    je .loadNextNote
    dec ax
    mov [musicDurationCounter], ax
    jmp .done
    
.loadNextNote:
    mov si, [currentMusicPtr]
    mov bx, [musicNoteIndex]
...
.playNote:
    mov cx, [si + bx + 2]
    mov [musicDurationCounter], cx
    
    add bx, 4
    mov [musicNoteIndex], bx
...
UpdateSoundEffect:
    ...
    mov ax, [soundEffectDuration]
    cmp ax, 0
    je .loadNextNote
    dec ax
    mov [soundEffectDuration], ax
    jmp .done
    
.loadNextNote:
    mov si, [soundEffectPtr]
    mov bx, [soundEffectIndex]
...
.playNote:
    mov cx, [si + bx + 2]
    mov [soundEffectDuration], cx
    
    add bx, 4
    mov [soundEffectIndex], bx
...
PlaySoundEffect:
    push ax
    mov [soundEffectPtr], si
    mov word [soundEffectIndex], 0
    mov word [soundEffectDuration], 0
    mov byte [soundEffectActive], 1
    pop ax
    ret
...
RestoreInterrupts:
    ...
    mov dx, word [oldTimerVector]
    mov ax, word [oldTimerVector + 2]
    mov ds, ax
    mov ax, 0x2508
    int 0x21
...
.gameOver:
    ...
    push si
    mov si, GameOverMusic
    call ChangeMusicTrack
    pop si
...
; ============ PROGRAM EXIT (for clean shutdown) ============
ExitProgram:
